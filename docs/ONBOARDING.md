...
You are taking over **Survey Segmenter**, a segmentation tool for survey data, built so that
someone who is not a statistician can run a defensible segmentation and be told plainly how far to
trust it. This
...
| `github.com/tomtomhz/survey-segmenter` | Public repo, MIT, owner `tomtomhz`. See `ci.yml` for what runs when. |
| A local "app for the team" folder | What non-developers are given: the signed `.zip`, `HOW TO RUN.md`, an example CSV. Not in the repo — it is a build output. |

**Never keep the checkout inside iCloud Drive** (on macOS, that includes `~/Desktop` and
`~/Documents` when Desktop & Documents syncing is on). The repo used to live there, and with ~6,000
files in `node_modules` every read went through the sync layer: `npm test` took **37 minutes**
instead of one second, Vitest workers timed out before starting, and macOS left `App 2.tsx`
conflict copies in the source tree. Roughly 25 minutes of a session was lost diagnosing this as a
code problem.
...
1. **Nothing containing real people's details belongs in this repo.** No respondent files, no
   contact lists, no exports from a live study — not in a commit, not in a test fixture, not in an
   issue. Everything committed as data is synthetic and generated by seeded code. If a task seems
   to require real data, stop and ask; the answer is a generator, not a file.
