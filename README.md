...
The honest remaining boundary is *upstream*: this tool clusters utilities or answers you already have. It does not estimate individual-level utilities from raw MaxDiff/conjoint choices (hierarchical Bayes) or design the experimental stimuli — the parts a full Moskowitz/Sawtooth pipeline is most known for. Feed it clean utilities (or categorical answers) and it will find and validate the segments to a professional standard; the estimation front-end is a separate tool.

## Versioning, deployment, and licensing

- **Version.** This is `segment_kmeans` version 1.0.0 (`segment_kmeans.__version__`, mirrored in `pyproject.toml`). Every report footer and `run_manifest.json` records the version, the exact config, the random seed, and the library versions, so any run reproduces exactly.
...
- **License.** No license file is included, because that is your decision to make. Add a `LICENSE` before sharing the code outside your team (for internal-only use, "all rights reserved" is the safe default; a permissive license such as MIT is common if you intend to share it).
